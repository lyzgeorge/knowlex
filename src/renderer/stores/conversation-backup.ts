...
function setupEventListeners() {
  ...
  function clearPendingChunks(messageId: string) {
    // Clear pending text chunks to prevent race condition with final message
    delete pendingTextChunks[messageId]
    delete pendingReasoningChunks[messageId]
  }
  ...
  on('message:streaming_end', (_, data: any) => {
    if (!data?.messageId || !data?.message) return
    // Clear pending chunks to prevent race condition with final message update
    clearPendingChunks(data.messageId)
    setState((s) => {
      s.isStreaming = false
      s.streamingMessageId = null
  ...
  on('message:streaming_error', (_, data: any) => {
    if (!data?.messageId || !data?.message) return
    // Clear pending chunks to prevent race condition with error message update
    clearPendingChunks(data.messageId)
    setState((s) => {
      s.isStreaming = false
      s.streamingMessageId = null
  ...
  on('message:streaming_cancelled', (_, data: any) => {
    ...
    // Clear pending chunks to prevent race condition with cancelled message update
    clearPendingChunks(data.messageId)
    setState((s) => {
      s.isStreaming = false
      s.streamingMessageId = null
